...
import { applyLineageUpdates } from "@/app/actions/member";
...
export default function LineageManager({
    ...
    const handleCompute = () => {
        ...
        try {
            ...
            const result: ComputedUpdate[] = persons.map((p) => {
                ...
                const oldGen = p.generation ?? null;
                const oldOrder = p.birth_order ?? null;
                return {
                    id: p.id,
                    full_name: p.full_name,
                    old_generation: oldGen,
                    new_generation: newGen,
                    old_birth_order: oldOrder,
                    new_birth_order: newOrder,
                    changed: newGen !== oldGen || newOrder !== oldOrder,
                };
            });
    ...
    const handleApply = async () => {
        ...
        try {
            const changedOnly = updates
                .filter((u) => u.changed)
                .map((u) => ({
                    id: u.id,
                    new_generation: u.new_generation,
                    new_birth_order: u.new_birth_order,
                }));

            await applyLineageUpdates(changedOnly);
                            ...
                            <table className="w-full text-sm">
                                ...
                                <tbody>
                                    {displayedRows.map((u, i) => (
                                        <tr
                                            key={u.id}
                                            className={`border-b border-stone-100 last:border-0 transition-colors ${u.changed ? "bg-amber-50/40" : ""
